...
class DatabaseLogger:
    ...
    def setup_event_listeners(self, engine: Engine):
        ...
        event.listen(engine.sync_engine, "after_cursor_execute", self._after_cursor_execute)
    ...
    def _sanitize_parameters(self, parameters) -> Dict[str, Any]:
        ...
        if isinstance(parameters, dict):
            ...
            return [str(param)[:100] if param else None for param in parameters[:10]]  # Limit array size
        else:
    ...
    def log_transaction_rollback(self, session: AsyncSession, exception: Exception = None):
