...
class SensitiveDataFilter(logging.Filter):
    ...
    SENSITIVE_FIELDS = {
        ...
        'refresh_token', 'access_token', 'stripe_secret', 'credit_card',
        'card_number', 'cvv', 'ssn', 'social_security', 'api_key'
    }
...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        formatted = (
            f"{color}[{record.asctime}] {record.levelname:8}{self.RESET} | "
...
def setup_logging() -> None:
    ...
    logging_config: Dict[str, Any] = {
        ...
        "loggers": {
            ...
            "app.database": {
                "level": "DEBUG",
                "handlers": ["database_file"] + (["console"] if log_level == "DEBUG" else []),
                "propagate": False,
            },
            "app.booking": {
                "level": log_level,
    ...
    logger.info("Comprehensive logging system initialized", 
                environment=settings.ENVIRONMENT, 
                log_level=log_level)
    logger.log_event("system.startup", 
                     environment=settings.ENVIRONMENT,
