...
class AdminService:
    ...
    async def get_users(
        ...
        query = select(User).options(
            selectinload(User.user_packages),
    ...
    async def get_dashboard_analytics(self) -> Dict[str, Any]:
        ...
        # Popular packages
        popular_packages_stmt = select(
            Package.name,
            func.count(UserPackage.id).label('purchase_count')
        ).join(UserPackage).group_by(Package.id, Package.name).order_by(desc('purchase_count')).limit(5)
        
        result = await self.db.execute(popular_packages_stmt)
        popular_packages = [{"name": row[0], "count": row[1]} for row in result.fetchall()]
    ...
    async def get_revenue_report(
        ...
        ).group_by(func.date(Payment.created_at)).order_by('date')
        ...
        # Revenue by package
        revenue_by_package_stmt = select(
            Package.name,
            func.sum(Payment.amount).label('revenue'),
            func.count(Payment.id).label('count')
        ).join(UserPackage).join(Payment).where(
            and_(
                Payment.created_at >= start_date,
                Payment.created_at <= end_date
            )
        ).group_by(Package.id, Package.name).order_by(desc('revenue'))
    ...
    async def get_attendance_report(
        ...
        bookings_by_date = [
            {"date": str(row[0]), "bookings": row[1]}
            for row in result.fetchall()
        ]
        
        # Popular class times
        popular_times_stmt = select(
        ...
        ).group_by(ClassInstance.start_time).order_by(desc('bookings')).limit(10)
        
        result = await self.db.execute(popular_times_stmt)
        popular_times = [
            {"time": str(row[0]), "bookings": row[1]}
            for row in result.fetchall()
        ]
