...
class CreditService:
    def __init__(self, db: AsyncSession):
        self.db = db
        self.logger = get_logger("app.credit")

    async def get_user_credit_balance(self, user_id: int) -> int:
        """Get the total credit balance for a user across all valid packages."""
        try:
            self.logger.debug("Retrieving user credit balance", user_id=str(user_id))

            stmt = select(func.sum(UserPackage.credits_remaining)).where(
                and_(
            ...
            final_balance = balance or 0

            self.logger.debug(
                "User credit balance retrieved successfully",
                user_id=str(user_id),
            ...
            self.logger.error(
                "Failed to retrieve user credit balance",
                exc_info=True,
                user_id=str(user_id),
                error=str(e),
            )
    ...
    async def use_credits_for_booking(
        self, user_id: int, booking_id: int, credits_needed: int = 1
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not user_packages:
                self.logger.warning(
                    "No valid packages found for credit deduction",
                    user_id=str(user_id),
                    booking_id=booking_id,
                )
                return {
            ...
            total_available = sum(
                float("inf") if pkg.package.is_unlimited else pkg.credits_remaining
                for pkg in user_packages
            )

            if total_available < credits_needed and not any(
                pkg.package.is_unlimited for pkg in user_packages
            ):
                self.logger.warning(
                    ...
                    booking_id=booking_id,
                    credits_needed=credits_needed,
                    credits_available=int(total_available),
                )
                return {
                    "success": False,
            ...
            if not selected_package:
                self.logger.warning(
                    "No suitable package found for credit deduction",
                    user_id=str(user_id),
                    booking_id=booking_id,
                    packages_count=len(user_packages),
                )
                return {
                    "success": False,
            ...
            business_logger.log_package_credit_used(
                user_id=str(user_id),
                package_id=str(selected_package.id),
                credits_used=credits_needed,
                remaining_credits=selected_package.credits_remaining,
                used_for=f"booking_{booking_id}",
            )
            ...
            self.logger.error(
                "Credit deduction failed with exception",
                exc_info=True,
                user_id=str(user_id),
                booking_id=booking_id,
                credits_needed=credits_needed,
    ...
    async def refund_credits_for_booking(
        self, user_id: int, booking_id: int, user_package_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not original_transaction:
                self.logger.warning(
                    "No deduction transaction found for refund",
                    user_id=str(user_id),
                    booking_id=booking_id,
                )
                return {
            ...
            user_package = await self.db.get(
                UserPackage, original_transaction.user_package_id
            )
            if not user_package:
                self.logger.warning(
                    "User package not found for refund",
                    user_id=str(user_id),
                    booking_id=booking_id,
                    package_id=original_transaction.user_package_id,
                )
                return {
                    "success": False,
            ...
            if user_package.is_expired:
                self.logger.warning(
                    "Cannot refund credits to expired package",
                    user_id=str(user_id),
                    booking_id=booking_id,
                    package_id=str(user_package.id),
                    expiry_date=user_package.expiry_date,
                )
                return {
                    "success": False,
            ...
            # Log successful credit refund
            business_logger.log_event(
                "package.credit_refunded",
                user_id=str(user_id),
                package_id=str(user_package.id),
            ...
            self.logger.info(
                ...
                credits_refunded=credits_to_refund,
                package_id=str(user_package.id),
                new_balance=user_package.credits_remaining,
            ...
            self.logger.error(
                "Credit refund failed with exception",
                exc_info=True,
                user_id=str(user_id),
                booking_id=booking_id,
                error=str(e),
    ...
    async def expire_packages(self) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info("Starting package expiration process")
            ...
            self.logger.info("Found packages to expire", count=len(expired_packages))
            ...
            for package in expired_packages:
                if package.credits_remaining > 0 and not package.package.is_unlimited:
                    ...
                    business_logger.log_package_expired(
                        user_id=str(package.user_id),
                        package_id=str(package.id),
                        unused_credits=package.credits_remaining,
                    )

                    self.logger.info(
                        "Package expired with credits",
                        user_id=str(package.user_id),
                        package_id=str(package.id),
                        unused_credits=package.credits_remaining,
            ...
            self.logger.info(
                "Package expiration process completed",
            ...
            self.logger.error(
                "Package expiration process failed", exc_info=True, error=str(e)
            )
            raise
                ...
                func.sum(
                    func.case(
                        (
                            Transaction.transaction_type
                            == TransactionType.CREDIT_DEDUCTION,
                            func.abs(Transaction.credit_amount),
                        ),
                        else_=0,
                    )
                ),
