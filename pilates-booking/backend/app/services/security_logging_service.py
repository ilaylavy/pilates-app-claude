...
class SecurityEventLogger:
    ...
    def log_security_event(self, event_type: SecurityEventType, threat_level: SecurityThreatLevel,
                          user_id: str = None, client_ip: str = None, user_agent: str = None,
                          **additional_data):
    ...
    def log_login_attempt(self, email: str, success: bool, client_ip: str, 
                         user_agent: str, user_id: str = None, failure_reason: str = None):
    ...
    def log_permission_denied(self, user_id: str, requested_resource: str, 
                            required_permission: str, client_ip: str, user_agent: str):
        """Log permission denied events."""
        
        threat_level = SecurityThreatLevel.HIGH if "admin" in requested_resource else SecurityThreatLevel.MEDIUM
    ...
    def log_admin_access(self, admin_id: str, endpoint: str, action: str,
                        client_ip: str, user_agent: str, target_data: Dict = None):
    ...
    def log_rate_limit_exceeded(self, endpoint: str, client_ip: str, 
    ...
    def log_attack_attempt(self, attack_type: SecurityEventType, payload: str,
                          client_ip: str, user_agent: str, endpoint: str = None):
    ...
    def log_data_access(self, user_id: str, data_type: str, operation: str,
                       client_ip: str, record_count: int = None, sensitive: bool = False):
        """Log data access events."""
        
        threat_level = SecurityThreatLevel.MEDIUM if sensitive else SecurityThreatLevel.LOW
        ...
        if record_count and record_count > 100:  # Bulk access
            ...
            event_type = SecurityEventType.SENSITIVE_DATA_ACCESS if sensitive else SecurityEventType.USER_DATA_MODIFICATION
    ...
    def _store_for_analysis(self, event_type: SecurityEventType, client_ip: str,
                           user_id: str, event_data: Dict):
        """Store event data for pattern analysis."""
        try:
            # Store by IP for IP-based analysis
            if client_ip:
                ip_key = f"security:ip:{client_ip}"
                self.redis_client.lpush(ip_key, f"{event_type.value}:{datetime.utcnow().timestamp()}")
            ...
            if user_id:
                ...
                self.redis_client.lpush(user_key, f"{event_type.value}:{datetime.utcnow().timestamp()}")
    ...
    def _track_failed_login(self, email: str, client_ip: str):
        ...
        try:
            ...
            # Track by email
            email_key = f"failed_logins:email:{hashlib.sha256(email.encode()).hexdigest()}"
    ...
    def _analyze_patterns(self, event_type: SecurityEventType, client_ip: str, user_id: str):
...
def log_login_attempt(email: str, success: bool, client_ip: str, 
                     user_agent: str, user_id: str = None, failure_reason: str = None):
    """Convenience function to log login attempts."""
    security_logger.log_login_attempt(email, success, client_ip, user_agent, user_id, failure_reason)


def log_admin_access(admin_id: str, endpoint: str, action: str,
                    client_ip: str, user_agent: str, target_data: Dict = None):
    """Convenience function to log admin access."""
    security_logger.log_admin_access(admin_id, endpoint, action, client_ip, user_agent, target_data)


def log_permission_denied(user_id: str, requested_resource: str, 
                         required_permission: str, client_ip: str, user_agent: str):
    """Convenience function to log permission denied events."""
    security_logger.log_permission_denied(user_id, requested_resource, required_permission, client_ip, user_agent)


def log_attack_attempt(attack_type: SecurityEventType, payload: str,
                      client_ip: str, user_agent: str, endpoint: str = None):
    """Convenience function to log attack attempts."""
    security_logger.log_attack_attempt(attack_type, payload, client_ip, user_agent, endpoint)