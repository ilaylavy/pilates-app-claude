...
class StripeService:
    ...
    async def create_or_get_customer(user: User, db: AsyncSession) -> str:
        ...
        try:
            logger.info(
                "Creating or retrieving Stripe customer",
                user_id=str(user.id),
                email=user.email,
            ...
            if user.stripe_customer_id:
                ...
                try:
                    ...
                    logger.warning(
                        "Existing Stripe customer ID invalid, creating new one",
                        user_id=str(user.id),
                        invalid_customer_id=user.stripe_customer_id,
                    )
                    pass
            ...
            customer = stripe.Customer.create(
                ...
                metadata={"user_id": str(user.id), "environment": settings.ENVIRONMENT},
            )
            ...
            logger.info(
                "Created new Stripe customer successfully",
                user_id=str(user.id),
                email=user.email,
                customer_id=customer.id,
            )

            # Log business event for customer creation
            ...
            logger.error(
                "Failed to create/get Stripe customer",
                exc_info=True,
                user_id=str(user.id),
                email=user.email,
                error=str(e),
    ...
    async def create_payment_intent(
        ...
        try:
            ...
            if user_package_id:
                payment_intent_data["metadata"]["user_package_id"] = str(
                    user_package_id
                )
            ...
            logger.info(
                "Created Stripe payment intent successfully",
            ...
            # Log business event for payment initiation
            business_logger.log_event(
                "payment.initiated",
                payment_intent_id=payment_intent.id,
                customer_id=customer_id,
    ...
    async def confirm_payment_intent(payment_intent_id: str) -> stripe.PaymentIntent:
        ...
        try:
            ...
            logger.error(
                f"Failed to confirm payment intent {payment_intent_id}: {str(e)}"
            )
            raise
    ...
    async def create_subscription(
        customer_id: str, price_id: str, metadata: Optional[Dict[str, Any]] = None
    ) -> stripe.Subscription:
