...
def upgrade() -> None:
    ...
    sa.Column('level', sa.Enum('BEGINNER', 'INTERMEDIATE', 'ADVANCED', 'ALL_LEVELS', name='classlevel'), nullable=True),
    ...
    sa.Column('status', sa.Enum('SCHEDULED', 'CANCELLED', 'COMPLETED', name='classstatus'), nullable=True),
    sa.Column('actual_capacity', sa.Integer(), nullable=True),
    sa.Column('notes', sa.Text(), nullable=True),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.ForeignKeyConstraint(['instructor_id'], ['users.id'], name=op.f('fk_class_instances_instructor_id_users')),
    sa.ForeignKeyConstraint(['template_id'], ['class_templates.id'], name=op.f('fk_class_instances_template_id_class_templates')),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_class_instances'))
    )
    op.create_index(op.f('ix_class_instances_id'), 'class_instances', ['id'], unique=False)
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.ForeignKeyConstraint(['package_id'], ['packages.id'], name=op.f('fk_user_packages_package_id_packages')),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('fk_user_packages_user_id_users')),
    ...
    sa.Column('status', sa.Enum('CONFIRMED', 'CANCELLED', 'NO_SHOW', 'COMPLETED', name='bookingstatus'), nullable=True),
    ...
    sa.Column('cancellation_reason', sa.Enum('USER_CANCELLED', 'CLASS_CANCELLED', 'NO_SHOW', 'ADMIN_CANCELLED', name='cancellationreason'), nullable=True),
    sa.Column('notes', sa.Text(), nullable=True),
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.ForeignKeyConstraint(['class_instance_id'], ['class_instances.id'], name=op.f('fk_bookings_class_instance_id_class_instances')),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('fk_bookings_user_id_users')),
    sa.ForeignKeyConstraint(['user_package_id'], ['user_packages.id'], name=op.f('fk_bookings_user_package_id_user_packages')),
    ...
    sa.Column('payment_type', sa.Enum('PACKAGE_PURCHASE', 'SINGLE_CLASS', 'LATE_CANCELLATION_FEE', 'NO_SHOW_FEE', 'REFUND', name='paymenttype'), nullable=False),
    sa.Column('payment_method', sa.Enum('CREDIT_CARD', 'CASH', 'BANK_TRANSFER', 'PAYPAL', 'STRIPE', name='paymentmethod'), nullable=False),
    sa.Column('status', sa.Enum('PENDING', 'COMPLETED', 'FAILED', 'REFUNDED', 'CANCELLED', name='paymentstatus'), nullable=True),
    ...
    sa.ForeignKeyConstraint(['user_package_id'], ['user_packages.id'], name=op.f('fk_payments_user_package_id_user_packages')),
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.ForeignKeyConstraint(['class_instance_id'], ['class_instances.id'], name=op.f('fk_waitlist_entries_class_instance_id_class_instances')),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('fk_waitlist_entries_user_id_users')),
